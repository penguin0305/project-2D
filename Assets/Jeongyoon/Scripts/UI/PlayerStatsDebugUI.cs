...
public class PlayerStatsDebugUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            row += lineHeight;
            GUI.Label(new Rect(col, row, columnWidth, 24), $"CritRate:   {s.CritRate * 100f:F1}%", statStyle);
            row += lineHeight;
            GUI.Label(new Rect(col, row, columnWidth, 24), $"CritDamage: {s.CritDamage * 100f:F0}%", statStyle);
        }
    }
}